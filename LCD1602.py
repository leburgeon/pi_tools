...
class CharLCD1602(object):
    ...
    def write_word(self, addr, data):
        """
        Writes a single byte of data to the specified I2C address.
    ...
    def send_command(self, comm):
        """
        Sends an instruction command to the LCD controller (e.g., clear screen, move cursor).
    ...
    def send_data(self, data):
        """
        Sends a single character (data) to be displayed on the LCD.
        
        Similar to send_command, this splits the 8-bit data into two 4-bit nibbles.
    ...
    def i2c_scan(self):
        """
        Scans the I2C bus (bus 1) for connected devices using system shell commands.
        
        Returns:
            list: A list of strings containing the hex addresses of found I2C devices.
        """
    ...
    def init_lcd(self, addr=None, bl=1):
        """
        Initializes the LCD hardware and prepares it for displaying data.
        ...
        to put the screen into 4-bit mode, configures the display settings, and clears it.
        
        Args:
            addr (int, optional): A specific I2C address to use. Defaults to None (auto-detect).
            bl (int, optional): Backlight state (1 for ON, 0 for OFF). Defaults to 1.
            
        Returns:
            bool: True if initialization was successful, False if an error occurred.
            
        Raises:
            IOError: If no valid I2C address (0x27 or 0x3f) is found on the bus.
        """
    ...
    def clear(self):
        """
        Clears the entire LCD screen and returns the cursor to the home position.
        """
        self.send_command(0x01) # Clear Screen

    def openlight(self):
        """
        Hardcodes a command to manually turn on the backlight via I2C address 0x27,
    ...
    def write(self, x, y, str):
        """
        Writes a text string to the LCD starting at a specific x, y coordinate.
        ...
        Args:
            ...
            str (str): The string of text to display.
        """
        if x < 0:
            x = 0
        if x > 15:
        ...
        # Row 0 starts at offset 0x00, Row 1 starts at offset 0x40.
        addr = 0x80 + 0x40 * y + x
    ...
    def display_num(self, x, y, num):
        """
        Writes a single ASCII character/number at a specific x, y coordinate.
